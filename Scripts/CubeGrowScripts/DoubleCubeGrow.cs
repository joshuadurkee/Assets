...
public class DoubleCubeGrow : AbstractCubeGrow {
	...
	void growCube()
	{
			 ...
			 player.transform.position.z < (this.gameObject.transform.parent.position + offset).z + 2 ))
		{
			...
			createdCube = (GameObject)MonoBehaviour.Instantiate(newCube, this.gameObject.transform.parent.position + (offset*2), Quaternion.identity);
			createdCube.transform.parent = newCubeParent.transform;
		}
		else { print("Fail"); }
	}
